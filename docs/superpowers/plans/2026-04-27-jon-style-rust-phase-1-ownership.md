...
```json
{
  ...
  "category": "Variance",
  "source": "Rust for Rustaceans, ch. 1 §Variance and Subtyping; Crust of Rust: Subtyping and Variance",
  "triggers": ["variance", "covariance", "contravariance", "invariant", "PhantomData lifetime", "variance marker"],
  "rule_summary": "Pick the `PhantomData<...>` marker that captures what your type semantically holds, because variance is what determines soundness. `PhantomData<&'a T>` is covariant in `'a` and `T` (read-like). `PhantomData<&'a mut T>` is invariant (exclusive-write). `PhantomData<fn() -> T>` is contravariant in `T` (consumer-like). `PhantomData<*const T>` and `PhantomData<*mut T>` are invariant in `T`.",
  "prompt_injection": "MANDATORY RULE: When defining a struct that conceptually holds a borrow or pointer it does not store as a field, choose the `PhantomData<...>` marker by variance, not by convenience. The marker dictates whether `'short` lifetimes can subtype `'long`, which determines whether your unsafe abstraction is sound under the borrow checker.",
  "anti_pattern": "use std::marker::PhantomData;\nstruct Slice<'a, T> {\n    ptr: *const T,\n    len: usize,\n    _marker: PhantomData<T>, // Wrong: ignores 'a; treats T as owned, not borrowed\n}",
  "good_pattern": "use std::marker::PhantomData;\n// Behaves like &'a [T]: covariant in 'a and T.\nstruct Slice<'a, T> {\n    ptr: *const T,\n    len: usize,\n    _marker: PhantomData<&'a T>,\n}\n// Behaves like &'a mut [T]: invariant in 'a and T.\nstruct SliceMut<'a, T> {\n    ptr: *mut T,\n    len: usize,\n    _marker: PhantomData<&'a mut T>,\n}\n// Consumer-like callback: contravariant in T.\nstruct Sink<T> {\n    f: fn(T),\n    _marker: PhantomData<fn(T)>,\n}"
}
```
