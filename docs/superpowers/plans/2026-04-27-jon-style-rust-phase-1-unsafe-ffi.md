...
```json
{
  ...
  "triggers": ["SAFETY comment", "unsafe block invariants", "undocumented_unsafe_blocks", "audit unsafe", "// SAFETY:", "justify unsafe"],
...
```json
{
  ...
  "rule_summary": "Declaring `unsafe fn foo()` says \"calling this requires invariants the type system cannot check.\" Document those preconditions in a `# Safety` rustdoc section above the function; callers must repeat them in their own `// SAFETY:` comment at each call site. Don't mark a function `unsafe` defensively — only mark it when a caller can break memory safety by misusing it. `clippy::missing_safety_doc` flags `pub unsafe fn` without `# Safety` docs.",
...
```json
{
  ...
  "good_pattern": "/// # Safety\n/// `mid <= s.len()` must hold.\nunsafe fn split_at_mut_unchecked<T>(s: &mut [T], mid: usize) -> (&mut [T], &mut [T]) {\n    let len = s.len();\n    let ptr = s.as_mut_ptr();\n    // SAFETY: caller guarantees mid <= len; ptr is valid for len elements; the\n    // two halves are disjoint by construction so no aliasing &mut is created.\n    let left = unsafe { std::slice::from_raw_parts_mut(ptr, mid) };\n    // SAFETY: same caller guarantee; ptr.add(mid) is in-bounds because mid <= len.\n    let right = unsafe { std::slice::from_raw_parts_mut(ptr.add(mid), len - mid) };\n    (left, right)\n}"
}
```
...
```json
{
  ...
  "rule_summary": "`mem::uninitialized<T>()` is undefined behavior for almost every `T` and is deprecated; reach for `MaybeUninit<T>` instead. Pattern: `let mut x = MaybeUninit::<T>::uninit();` then write each field through `addr_of_mut!(x.as_mut_ptr().as_mut().unwrap_or_unreachable().field)` (or `(*x.as_mut_ptr()).field = ...` inside `unsafe`), and only call `assume_init()` once *every* field is initialized. Critical: never construct a `&mut T` to a `MaybeUninit<T>` and write fields through it — the `&mut T` itself is invalid until `T` is fully initialized, regardless of whether you read from it.",
  "prompt_injection": "MANDATORY RULE: For delayed initialization use `MaybeUninit<T>`, not `mem::uninitialized` (deprecated, immediate UB for non-zero-sized non-Copy types). Write fields through raw pointers (`addr_of_mut!` or `(*x.as_mut_ptr()).field = ...`); never materialize a `&mut T` while the value is partially initialized. Call `assume_init()` only after every field has been written.",
...
```json
{
  ...
  "triggers": ["UnsafeCell", "interior mutability primitive", "shared mutability", "Cell RefCell Mutex source", "&T to &mut T"],
...
```json
{
  ...
  "rule_summary": "Three rules at the `extern \"C\"` boundary: (1) Rust panics that unwind into foreign code are undefined behavior under the default `extern \"C\"` ABI — wrap every `extern \"C\" fn` body in `std::panic::catch_unwind` and convert `Err` into an error code or sentinel value. The opt-in `extern \"C-unwind\"` ABI (stabilized 1.71) explicitly permits unwinding for callers that want it. (2) Never expose Rust references (`&T`/`&mut T`) across the boundary — C cannot uphold validity invariants. Use `*const T`/`*mut T` and document the lifetime/ownership rules. (3) `extern \"C\"` functions cannot be generic; instantiate concrete wrappers per type.",
  "prompt_injection": "MANDATORY RULE: Every `extern \"C\" fn` body wraps its work in `std::panic::catch_unwind` and converts panics into a sentinel error value. Function signatures use `*const T`/`*mut T`, never `&T`/`&mut T`, and the rustdoc above the function documents who allocates, who frees, and what null/length conventions hold. No generic `extern \"C\"` functions — write concrete `extern \"C\" fn parse_u32_buffer(...)` wrappers around generic helpers.",
  "anti_pattern": "// Three problems: panic across boundary is UB, &mut MyType is exposed,\n// and the function is generic (won't compile, but the intent is wrong).\n#[unsafe(no_mangle)]\npub extern \"C\" fn process<T>(value: &mut T) -> i32 {\n    panic!(\"unimplemented\"); // UB if the caller is C\n}",
