...
```json
{
  ...
  "anti_pattern": "// Single named lifetime: caller must produce &'a T whose 'a outlives the function call.\n// Breaks once the implementation has only a local borrow to pass to the callback.\nfn first_match<'a, F>(s: &'a str, f: F) -> Option<&'a str>\nwhere F: Fn(&'a str) -> bool { /* ... */ }",
  "good_pattern": "// HRTB: callback works for any lifetime the implementation chooses.\nfn first_match<F>(s: &str, f: F) -> Option<String>\nwhere F: for<'a> Fn(&'a str) -> bool {\n    if f(s) { Some(s.to_owned()) } else { None }\n}"
}
```
