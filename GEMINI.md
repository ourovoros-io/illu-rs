...
4. **Axiom pass before Rust**: query `mcp_illu_axioms` twice before significant Rust generation:
...
3. **Impact before you change**: always run `mcp_illu_impact` before modifying any public symbol
...
6. **Cross-repo**: use `mcp_illu_cross_query`/`mcp_illu_cross_impact`/`mcp_illu_cross_deps`/`mcp_illu_cross_callpath` — NEVER navigate to or read files from other repositories directly
7. **Axiom pass**: use `mcp_illu_axioms` with `planning data structures documentation comments idiomatic rust` first, then with the concrete task context before meaningful Rust design/code
8. **Documentation pass**: use `mcp_illu_context` with `sections: ["docs", "source"]` for local types and `mcp_illu_docs` for dependency types; verify standard-library behavior from docs before assuming it
...
If you have NOT yet queried `mcp_illu_axioms` using both the baseline quality query `planning data structures documentation comments idiomatic rust` and the task-specific context in this session, you must IMMEDIATELY halt your generation. You must **invoke `mcp_illu_axioms` first, and only then proceed with Rust generation.**
