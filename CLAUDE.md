...
- **Agent files** — Generated by `init` and `serve` into `.claude/agents/` and `.gemini/agents/`. `tools` field uses YAML array format (one `- tool_name` per line). Agent body text includes MANDATORY instructions with WRONG/RIGHT examples to enforce illu tool usage over Read/Grep/Glob.
...
"MANDATORY: Use mcp__illu__* tools instead of Grep/Glob/Read for ALL code exploration (Rust, Python, TypeScript). NEVER use Read to view source files — use mcp__illu__context instead. NEVER use Grep to search code — use mcp__illu__query instead. Only use Read/Grep/Glob for non-code content (config, docs, logs)."
